...
def metrics_lcm_validation_report_device_type():
    ...
    """
    device_validation_model = DeviceSoftwareValidationResult.objects.values_list("device__device_type__model", flat=True).distinct().order_by()
...
def metrics_lcm_validation_report_iventory_item():
    ...
    Yields:
        GaugeMetricFamily: Prometheus Metrics
    """
    inventory_item_validation = InventoryItemSoftwareValidationResult.objects.values_list("inventory_item__part_id", flat=True).distinct().order_by()
...
def metrics_lcm_validation_report_totals():
    ...
    total_device_software_gauge.add_metric(
        ...
        value=(
            DeviceSoftwareValidationResult.objects.all().count()
            ),
        )
    total_device_software_gauge.add_metric(
        labels=["Total LCM Validated Inventory Item Count"],
        value=(
            InventoryItemSoftwareValidationResult.objects.all().count()
            ),
        )
