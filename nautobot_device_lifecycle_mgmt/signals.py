...
def post_migrate_create_relationships(sender, apps=global_apps, **kwargs):  # pylint: disable=unused-argument
    ...
    for relationship_dict in [{"name": "Software on Device",
                               "slug": "device_soft",
        ...
        _Relationship.objects.get_or_create(
...
def delete_softwarelcm_relationships(sender, instance, **kwargs):  # pylint: disable=unused-argument
    ...
    soft_relationships = Relationship.objects.filter(
        slug__in=("device_soft", "inventory_item_soft"))
    RelationshipAssociation.objects.filter(
        relationship__in=soft_relationships, source_id=instance.pk).delete()
...
def delete_device_software_relationship(sender, instance, **kwargs):  # pylint: disable=unused-argument
    ...
    soft_relationships = Relationship.objects.filter(
        slug__in=("device_soft", "inventory_item_soft"))
    RelationshipAssociation.objects.filter(
        relationship__in=soft_relationships,
        destination_id=instance.pk).delete()
