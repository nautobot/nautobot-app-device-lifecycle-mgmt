...
class TestVersion(unittest.TestCase):
    ...
    def test_version(self):
        ...
        poetry_version = toml.load(os.path.join(parent_path, "pyproject.toml"))[
            "tool"]["poetry"]["version"]
...
class TestDocsPackaging(unittest.TestCase):
    ...
    def test_version(self):
        ...
        parent_path = os.path.dirname(os.path.dirname(
            os.path.dirname(os.path.realpath(__file__))))
        poetry_details = toml.load(os.path.join(parent_path, "pyproject.toml"))[
            "tool"]["poetry"]["dev-dependencies"]
        with open(f"{parent_path}/docs/requirements.txt", "r", encoding="utf-8") as file:
            requirements = [line for line in file.read().splitlines() if (
